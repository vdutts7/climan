...
1. `GET /`, `/pwsh`, `/pwsh/{cmdlet}`, `/ps/{cmdlet}`, `/search`, `/robots.txt`
2. no POST, PUT, PATCH, DELETE
3. no admin routes, no upload endpoints, no dynamic code exec

Writes happen only via:

1. `seed_pwsh.py` with operator `CF_TOKEN` + `PGPASSWORD` (local machine)
2. Azure portal / `psql` with database credentials (operator only)
...
1. no injection surface- path regex constrained; parameterized SQL via `postgres` driver
2. no secrets in repo- credentials via `.env` only (gitignored)
3. no write bindings exposed to fetch handler
4. seed scripts require local env vars; not callable remotely
